/* 
SUMMARIZATION_PROMPT - 第一次把完整历史变成状态快照 固定格式 - 目标 / 保存限制条件 / 状态机 / 架构选择 / 未来行动 / 最重要的信息
SUMMARIZATION_SYSTEM_PROMPT - 限制模型只做状态总结 被总结的历史里面可能有用户问题、代码、指令，如果不限制，模型可能继续回答旧问题
UPDATE_SUMMARIZATION_PROMPT - 基于旧快照增量更新状态
TURN_PREFIX_SUMMARIZATION_PROMPT - 单轮过长时压缩前半段，保护后半段理解
*/
