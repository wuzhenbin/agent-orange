...
export const createGrepTool = (
    ...
    async execute(
        _toolCallId: string,
        { pattern, file_path, glob, ignoreCase, literal, context, limit },
        signal?: AbortSignal,
    ) {
        return new Promise((resolve, reject) => {
            ...
            ;(async () => {
                try {
                    ...
                    try {
                        isDirectory = await grepOperations.isDirectory(searchPath)
                    } catch {
                    ...
                    const getFileLines = async (filePath: string): Promise<string[]> => {
                        ...
                        if (!lines) {
                            try {
                                const content = await grepOperations.readFile(filePath)
                                lines = content
                                    .replace(/\r\n/g, "\n")
                                    .replace(/\r/g, "\n")
                                    .split("\n")
